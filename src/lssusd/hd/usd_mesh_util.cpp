...
namespace lss {
  ...
  void UsdMeshUtil::ComputeTriangleIndices(VtVec3iArray* indices, VtIntArray* primitiveParams, VtIntArray* edgeIndices/*=nullptr*/) const {
    ...
    for (int i = 0; i < numFaces; ++i) {
      const int nVerts = numVertsPtr[i];
      const bool isHole = holeIndex < numholeIndices && holeIndicesPtr[holeIndex] == i;
      if (isHole) {
        ++holeIndex;
      } else if (nVerts < 3) {
        // skip degenerated face
        invalidTopology = true;
      } else if (v < 0 || nVerts > numVertIndices - v) {
        invalidTopology = true;
      } else {
        numTris += nVerts - 2;
      }

      if (nVerts > 0 && v <= numVertIndices) {
        v += std::min(nVerts, numVertIndices - v);
      } else {
        invalidTopology = true;
      }
    }
    ...
    for (int i = 0, tv = 0, v = 0, ev = 0; i < numFaces; ++i) {
      ...
      const bool faceRangeValid = nv >= 3 && v >= 0 && nv <= numVertIndices - v;
      if (holeIndex < numholeIndices && holeIndicesPtr[holeIndex] == i) {
        // Skip hole faces.
        ++holeIndex;
      } else if (nv < 3) {
        // Skip degenerate faces.
      } else if (!faceRangeValid) {
        invalidTopology = true;
      } else {
      ...
      if (nv > 0 && v <= numVertIndices) {
        const int safeNv = std::min(nv, numVertIndices - v);
        v += safeNv;
        ev += safeNv;
      } else {
        invalidTopology = true;
  ...
  static
    ...
    for (int i = 0; i < (int) faceVertexCounts.size(); ++i) {
      ...
      const bool isHole = holeIndex < numholeIndices && holeIndices[holeIndex] == i;
      if (isHole) {
        ++holeIndex;
      } else if (nVerts < 3) {
        // skip degenerated face
        invalidTopology = true;
      } else if (v < 0 || nVerts > numElements - v) {
        invalidTopology = true;
      } else {
        numFVarValues += 3 * (nVerts - 2);
      }

      if (nVerts > 0 && v <= numElements) {
        v += std::min(nVerts, numElements - v);
      } else {
        invalidTopology = true;
      }
    }
    ...
    for (int i = 0, v = 0; i < (int) faceVertexCounts.size(); ++i) {
      ...
      const bool faceRangeValid = nVerts >= 3 && v >= 0 && nVerts <= numElements - v;

      if (holeIndex < numholeIndices && holeIndices[holeIndex] == i) {
        // Skip hole faces.
        ++holeIndex;
      } else if (nVerts < 3) {
        // Skip degenerate faces.
