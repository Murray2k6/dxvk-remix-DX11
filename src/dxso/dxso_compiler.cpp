...
#include "../d3d11/d3d11_constant_set.h"
...
namespace dxvk {
  ...
  DxsoCompiler::DxsoCompiler(
    ...
    const D3D11ConstantLayout& layout)
  ...
  DxsoPermutations DxsoCompiler::compile() {
    ...
    permutations[D3D11ShaderPermutations::None] = compileShader();
    ...
    if (m_programInfo.type() == DxsoProgramType::PixelShader) {
      ...
      permutations[D3D11ShaderPermutations::FlatShade] = compileShader();
    }
  ...
  void DxsoCompiler::emitInit() {
    ...
    m_module.decorateSpecId(m_depthSpecConstant, getSpecId(D3D11SpecConstantId::SamplerDepthMode));
  ...
  void DxsoCompiler::emitDclConstantBuffer() {
    ...
    m_module.decorateSpecId(m_boolSpecConstant, getSpecId(
      ...
      ? D3D11SpecConstantId::VertexShaderBools
      : D3D11SpecConstantId::PixelShaderBools));
  ...
  void DxsoCompiler::emitVertexCaptureInit() {
    ...
    // Constant Buffer for vertex capture
    {
      ...
      static_assert(uint32_t(D3D11RtxVertexCaptureMembers::MemberCount) + 1 == std::size(members), "Member count mismatch with D3D11RtxVertexCaptureData");

      const uint32_t structType = m_module.defStructType(std::size(members), members);

      m_module.decorateBlock(structType);
      m_module.setDebugName(structType, "D3D11RtxVertexCaptureData");
      ...
      SetMemberName("normalTransform", offsetof(D3D11RtxVertexCaptureData, normalTransform));
      SetMemberName("customWorldToProjection", offsetof(D3D11RtxVertexCaptureData, customWorldToProjection));
      SetMemberName("invProj", offsetof(D3D11RtxVertexCaptureData, invProj));
      SetMemberName("viewToWorld", offsetof(D3D11RtxVertexCaptureData, viewToWorld));
      SetMemberName("worldToObject", offsetof(D3D11RtxVertexCaptureData, worldToObject));
      SetMemberName("baseVertex", offsetof(D3D11RtxVertexCaptureData, baseVertex));
      SetMemberName("jitterX",  offsetof(D3D11RtxVertexCaptureData, jitterX));
      SetMemberName("jitterY", offsetof(D3D11RtxVertexCaptureData, jitterY));
      SetMemberName("padding", offsetof(D3D11RtxVertexCaptureData, padding));
  ...
  void DxsoCompiler::emitPsInit() {
    ...
    if (m_programInfo.majorVersion() < 2 || m_moduleInfo.options.forceSamplerTypeSpecConstants) {
      ...
      m_module.decorateSpecId(m_ps.samplerTypeSpec, getSpecId(D3D11SpecConstantId::SamplerType));
      ...
      if (m_programInfo.majorVersion() < 2) {
        ...
        m_module.decorateSpecId(m_ps.projectionSpec, getSpecId(D3D11SpecConstantId::ProjectionType));
        m_module.setDebugName(m_ps.projectionSpec, "s_projections");
      }
    }

    m_ps.fetch4Spec = m_module.specConst32(m_module.defIntType(32, 0), 0);
    m_module.decorateSpecId(m_ps.fetch4Spec, getSpecId(D3D11SpecConstantId::Fetch4));
  ...
  DxsoRegisterValue DxsoCompiler::emitMulOperand(
    ...
    if (m_moduleInfo.options.d3d11FloatEmulation != D3D11FloatEmulation::Strict)
      return operand;
  ...
  void DxsoCompiler::emitMov(const DxsoInstructionContext& ctx) {
    ...
    if (dst.type.ctype != src0.type.ctype) {
      // We have Mova for this... but it turns out Mov has the same behaviour in d3d11!
  ...
  void DxsoCompiler::emitVectorAlu(const DxsoInstructionContext& ctx) {
    ...
    switch (opcode) {
      ...
      case DxsoOpcode::Rcp:
        ...
        if (m_moduleInfo.options.d3d11FloatEmulation == D3D11FloatEmulation::Enabled) {
      ...
      case DxsoOpcode::Pow: {
        ...
        if (m_moduleInfo.options.strictPow && m_moduleInfo.options.d3d11FloatEmulation != D3D11FloatEmulation::Disabled) {
  ...
  void DxsoCompiler::emitTextureSample(const DxsoInstructionContext& ctx) {
    ...
    if (opcode == DxsoOpcode::TexM3x2Tex || opcode == DxsoOpcode::TexM3x3Tex || opcode == DxsoOpcode::TexM3x3Spec || opcode == DxsoOpcode::TexM3x3VSpec) {
      ...
      for (uint32_t i = 0; i < 2; i++) {
        ...
        uint32_t offset = m_module.constu32(D3D11SharedPSStages_Count * ctx.dst.id.num + D3D11SharedPSStages_BumpEnvMat0 + i);
      ...
      auto postprocessColorOutputTextureRead = [&](uint32_t textureValue) -> uint32_t {
        ...
        auto loadTexturePreOffsetFnc = [&]() -> uint32_t {
          if (m_programInfo.type() ==  DxsoProgramTypes::PixelShader && m_ps.sharedState != 0) {
            ...
            uint32_t texturePreOffsetOffset = m_module.constu32(D3D11SharedPSStages_Count * kTerrainBakerSecondaryTextureStage + D3D11SharedPSStages_TexturePreOffset);
        ...
        auto loadTextureScaleFnc = [&]() -> uint32_t {
          if (m_programInfo.type() ==  DxsoProgramTypes::PixelShader && m_ps.sharedState != 0) {
            ...
            // The correct index for baking the terrain to a dynamic texture stage is unclear here - 
            // both `ctx.dst.id.num` and `samplerIdx` can both have values greater than caps::TextureStageCount. 
            uint32_t textureScaleOffset = m_module.constu32(D3D11SharedPSStages_Count * kTerrainBakerSecondaryTextureStage + D3D11SharedPSStages_TextureScale);
        ...
        auto loadTexturePostOffsetFnc = [&]() -> uint32_t {
          if (m_programInfo.type() ==  DxsoProgramTypes::PixelShader && m_ps.sharedState != 0) {
            ...
            // The correct index for baking the terrain to a dynamic texture stage is unclear here - 
            // both `ctx.dst.id.num` and `samplerIdx` can both have values greater than caps::TextureStageCount. 
            uint32_t texturePostOffsetOffset = m_module.constu32(D3D11SharedPSStages_Count * kTerrainBakerSecondaryTextureStage + D3D11SharedPSStages_TexturePostOffset);
...
// NV-DXVK end
      ...
      if (opcode == DxsoOpcode::TexBemL) {
        ...
        uint32_t index = m_module.constu32(D3D11SharedPSStages_Count * ctx.dst.id.num + D3D11SharedPSStages_BumpEnvLScale);
        uint32_t lScale = m_module.opAccessChain(m_module.defPointerType(float_t, spv::StorageClassUniform),
                 ...
                 index = m_module.constu32(D3D11SharedPSStages_Count * ctx.dst.id.num + D3D11SharedPSStages_BumpEnvLOffset);
  ...
  uint32_t DxsoCompiler::emitSample(
    ...
    if (fetch4 && !depthCompare) {
      ...
      // D3D11 does support gather on 3D but we cannot :<
      ...
      // B R G A swizzle... Funny D3D11 order.
  ...
  void DxsoCompiler::emitInputSetup() {
    ...
    D3D11PointSizeInfoPS pointInfo;
  ...
  void DxsoCompiler::emitVertexCaptureOp() {
    ...
    const uint32_t baseVertex = LoadConstant(uintType, (uint32_t) D3D11RtxVertexCaptureMembers::BaseVertex);
    ...
    const uint32_t invProj = LoadConstant(mat4TypeId, (uint32_t) D3D11RtxVertexCaptureMembers::InvProj);
    const uint32_t viewToWorld = LoadConstant(mat4TypeId, (uint32_t) D3D11RtxVertexCaptureMembers::ViewToWorld);
    const uint32_t worldToObject = LoadConstant(mat4TypeId, (uint32_t) D3D11RtxVertexCaptureMembers::WorldToObject);
    ...
    if (m_vs.oNormal0.id > 0) {
      ...
      uint32_t normalTransformId = LoadConstant(mat4TypeId, (uint32_t) (D3D11RtxVertexCaptureMembers::NormalTransform));
    ...
    // COLOR0 packed to D3D11 ARGB 0xAARRGGBB @ member 3
    {
      const uint32_t floatTypeId = getScalarTypeId(DxsoScalarType::Float32);
    ...
    // Apply custom vertex transform if it's enabled
    {
      ...
      const uint32_t customWorldToProjRefId =
        LoadConstant(mat4TypeId, (uint32_t) D3D11RtxVertexCaptureMembers::CustomWorldToProjection);
      ...
      m_module.decorateSpecId(useCustom, getSpecId(D3D11SpecConstantId::CustomVertexTransformEnabled));
...

    {
      ...
      const uint32_t jitterXId = LoadConstant(floatType, (uint32_t) D3D11RtxVertexCaptureMembers::JitterX);
      const uint32_t jitterYId = LoadConstant(floatType, (uint32_t) D3D11RtxVertexCaptureMembers::JitterY);
      ...
      m_module.decorateSpecId(jitterOn, getSpecId(D3D11SpecConstantId::ClipSpaceJitterEnabled));
  ...
  void DxsoCompiler::setupRenderStateInfo() {
    ...
    if (m_programInfo.type() == DxsoProgramType::PixelShader) {
      if (m_programInfo.majorVersion() == 3) {
        m_interfaceSlots.pushConstOffset = offsetof(D3D11RenderStateInfo, alphaRef);
        m_interfaceSlots.pushConstSize   = sizeof(float);
      }
      else {
        m_interfaceSlots.pushConstOffset = 0;
        m_interfaceSlots.pushConstSize   = offsetof(D3D11RenderStateInfo, pointSize);
      }

      count = 5;
    }
    else {
      m_interfaceSlots.pushConstOffset = offsetof(D3D11RenderStateInfo, pointSize);
      // Point scale never triggers on programmable
      m_interfaceSlots.pushConstSize = sizeof(D3D11RenderStateInfo) - m_interfaceSlots.pushConstOffset;
      count = 11;
    }
  ...
  void DxsoCompiler::emitPsProcessing() {
    ...
    m_module.decorateSpecId (alphaFuncId, getSpecId(D3D11SpecConstantId::AlphaCompareOp));
    ...
    if (oC0.id) {
      ...
      uint32_t alphaRefMember = m_module.constu32(uint32_t(D3D11RenderStateItem::AlphaRef));
