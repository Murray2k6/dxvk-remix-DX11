#!/usr/bin/env python3
# DX11_V167_FULL_SHADER_TREE_HIDDEN_MODULES
...
ORIG = SCRIPT_DIR / "compile_shaders_orig_v167.py"
for fallback_name in ("compile_shaders_orig_v167.py", "compile_shaders_orig_v167.py", "compile_shaders_orig_v167.py"):
    if ORIG.is_file():
        break
...
def main() -> int:
    if not ORIG.is_file():
        print(f"DX11_V167_FULL_SHADER_TREE_HIDDEN_MODULES: original compiler missing: {ORIG}", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory(prefix="dx11_v167_shader_fulltree_", ignore_cleanup_errors=True) as td:
        ...
        if output_dir is not None:
            ...
            report = output_dir / "dx11_v167_full_shader_tree_hidden_modules.txt"
            report.write_text(
                "DX11_V167_FULL_SHADER_TREE_HIDDEN_MODULES\n"
        ...
        print(
            "DX11_V167_FULL_SHADER_TREE_HIDDEN_MODULES: compiling full shader tree with shared modules hidden "
