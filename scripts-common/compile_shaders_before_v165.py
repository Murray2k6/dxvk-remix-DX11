#!/usr/bin/env python3
# DX11_V163_REAL_SHADER_INCLUDE_TREE
...
ORIG = SCRIPT_DIR / "compile_shaders_orig_v163.py"
if not ORIG.is_file():
    fallback = SCRIPT_DIR / "compile_shaders_orig_v163.py"
    if fallback.is_file():
        ORIG = fallback
...
def main() -> int:
    if not ORIG.is_file():
        print(f"DX11_V163_REAL_SHADER_INCLUDE_TREE: original compiler missing: {ORIG}", file=sys.stderr)
        return 2
    ...
    with tempfile.TemporaryDirectory(prefix="dx11_v163_shader_entries_") as td:
        ...
        if output_dir is not None:
            ...
            report = output_dir / "dx11_v163_skipped_slang_modules_and_copied_headers.txt"
            report.write_text(
                "DX11_V163_REAL_SHADER_INCLUDE_TREE\n"
        ...
        print(
            "DX11_V163_REAL_SHADER_INCLUDE_TREE: compiling filtered shader entry tree "
