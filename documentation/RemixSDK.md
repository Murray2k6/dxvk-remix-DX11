...
* **Direct3D 11 Translation Layer:** This fork intercepts the D3D11 API and uses draw-call state, resources, and transforms to reconstruct the scene hierarchy before passing it to Remix's path-traced renderer. The source code for this component is available in this repository.

* **Runtime Packaging:** The runtime ships as a D3D11-facing drop-in (`d3d11.dll` plus `dxgi.dll`) together with the supporting Remix renderer binaries. This fork targets x64 applications.

# Remix SDK Design

The Remix SDK is designed to give developers direct access to the advanced features of the Remix Renderer, bypassing the limits of automatic D3D11 draw-call capture when an application wants explicit control over materials, lights, meshes, and presentation.

It consists of a [plain C header](https://github.com/NVIDIAGameWorks/dxvk-remix/blob/main/public/include/remix/remix_c.h) file with a straightforward API, and an [optional C++ wrapper](https://github.com/NVIDIAGameWorks/dxvk-remix/blob/main/public/include/remix/remix.h) that can help reduce boilerplate code for projects written in C++. In this fork, the API implementation is part of the D3D11 runtime binaries.
...
**Note** that the runtime binaries built this way are identical to the regular builds of dxvk-remix, but the file layout generated by the copy_sdk step is not. However, downloading pre-built runtime binaries and headers from the repository can be an alternative starting point if you don’t want to build dxvk-remix locally.
...
1. To load the Remix Renderer library, call the helper function `remixapi_lib_loadRemixDllAndInitialize`. It loads the Remix Renderer's dynamic library (`.dll`), and returns a `remixapi_Interface` struct that contains all the RemixAPI functions.
    ...
    * When a target application (e.g. `.exe`) is being compiled with the API header, the version numbers `REMIXAPI_VERSION_*` are statically bound into the compiled target app binaries.
    That way, the target app is compatible only with specific versions of the Remix runtime binaries (`d3d11.dll` / `dxgi.dll`).
...
# Remix API in existing Direct3D 11 apps

While the given example is graphics-API agnostic, this fork integrates with D3D11.
The main constraint is that Remix API is 64-bit only, which means that the game also must be built for x64.

So given an existing 64-bit D3D11 application, here are the steps to integrate Remix API:

* Ensure that the app works with the original Remix Runtime
* In the D3D11 app's source files, there will be a device creation path such as `D3D11CreateDevice` or `D3D11CreateDeviceAndSwapChain`, so after that:

    1. Call `remixapi_lib_loadRemixDllAndInitialize(L"d3d11.dll")` to init the Remix API
    1. Call `remixapi_Interface::dxvk_RegisterD3D11Device`, passing the `ID3D11Device*`
