...
bool DeviceManager_VK::pickPhysicalDevice()
{
    ...
    for(const auto& dev : devices)
    {
        ...
        // If specfic adapter is requested, and this isn't it, skip it
        const auto deviceNameTmp = prop.deviceName.operator std::string();
        std::wstring deviceName = std::wstring(deviceNameTmp.cbegin(), deviceNameTmp.cend());
        if (m_DeviceParams.adapterNameSubstring.length() != 0)
        {
            if (deviceName.find(m_DeviceParams.adapterNameSubstring) == std::string::npos)
            {
                continue;
            }
        }
...
bool DeviceManager_VK::createDevice()
{
    ...
    rendererString = std::string(prop.deviceName.data());
...
bool DeviceManager_VK::CreateDeviceAndSwapChain()
{
    if (m_DeviceParams.enableDebugRuntime)
    {
        ...
        #ifndef _M_ARM64
        // In order to load validation layer in arm64 it would need to be compiled for arm64.
        // As of 2023/5, the only layer available in public SDK is x64, so disable it for now.
